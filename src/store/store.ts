...
export function applyStream(recipe: (draft: AppState) => void): void {
  useStore.setState(produce(useStore.getState(), recipe), true)
}

export function undo(): void {
  const e = history.popUndo()
  if (!e) return
  useStore.setState(applyPatches(useStore.getState(), e.inverse), true)
}

export function redo(): void {
  const e = history.popRedo()
  if (!e) return
  useStore.setState(applyPatches(useStore.getState(), e.patches), true)
}
